(function(){
    ...
    FZ.GameBase.setCss = function(div, info){
        //不显示关卡数和可提示次数
        if(info.name.indexOf("NUM_LEVEL")>-1 || info.name.indexOf("number_hint")>-1){
            div.style.display = "none";
            return;
        }
        //设置多背景
        if(info.name.indexOf("gameIn_BG")>-1){
            div.style.backgroundImage = "url(imgs/top.png),url(imgs/"+ info.fileURL +")";
        ...
        //设置背景图大小
        if(info.size){
            div.style.backgroundSize = info.size;
        }else{
        ...
        if(info.r && info.r !=0){
            div.style.right = info.r + "px";
        }
        if(info.b && info.b !=0){
            div.style.bottom = info.b + "px";
        }
    ...
    var createOrientationDiv = function(){
        ...
        ortDiv.style.backgroundSize = "cover";
